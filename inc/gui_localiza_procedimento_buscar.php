...
$gui_select_cidade_atend    = (empty($_POST['gui_select_cidade_atend'])) ? "" : verifica($_POST['gui_select_cidade_atend']);
...
if(!empty($sel_id_local_atendimento) AND $sel_id_local_atendimento > 0){
    $where_local_atendimento = "AND loc.id_local_atendimento = $sel_id_local_atendimento";
}

$where_cidade_atendimento = '';
if(!empty($gui_select_cidade_atend)){
    $where_cidade_atendimento = "AND loc_ate.cidade = '$gui_select_cidade_atend'";
}
...
$sql        = "SELECT pro.*, loc.id_convenio, loc.preco_custo, loc.preco_venda, loc_ate.id_local_atendimento, loc_ate.nome'nome_local_atendimento', loc_ate.cidade'cidade_local_atendimento' FROM gui_procedimentos pro
...
WHERE pro.ativo = 'S' AND loc_ate.ativo = 'S' AND loc.preco_venda <> '' $where_local_atendimento $where_cidade_atendimento $where_busca $where_grupo_procedimento $where_sel_procedimento $where_id_convenio_paciente_sel $where_grupo_convenio
    ...
    while ($dados = mysql_fetch_array($query))
    {
        ...
        if (mysql_num_rows($query_grupo)>0)
        {
            ...
            echo "<tr>
                ...
                <td><a href=\"inc/gui_ver_local_atendimento.php?id_local_atendimento=$id_local_atendimento\" data-target=\"#ajax\" data-toggle=\"modal\">$nome_local_atendimento</a> <span class='small bold'>$cidade_local_atendimento</span></td>
                <td>$nome_convenio</td>";
                if(in_array("42", $verifica_lista_permissoes_array_inc)){
