<?php
    ...
    $id_dependente  = (empty($_GET['id_dependente'])) ? "" : verifica($_GET['id_dependente']);
    ...
    $sql        = "SELECT nome'nome_dependente', data_nascimento'dt_nascimento_dependente' FROM dependentes_clientes 
    ...
    if (mysql_num_rows($query)>0)
    {
        ...
        $verifica           = "SELECT * FROM controle_impressao 
                            WHERE id_referencia = $id_dependente AND tipo_impressao = 'cartao_dependente'";
        $query_verifica    = mysql_query($verifica, $banco_produto);
             
        if (mysql_num_rows($query_verifica)>0)
        {
            $sql2    = "UPDATE controle_impressao SET data_impressao = '$agora' WHERE id_referencia = $id_dependente AND tipo_impressao = 'cartao_dependente'";
        ...
        else
        {
            $sql3    = "INSERT INTO controle_impressao (tipo_impressao, id_referencia, data_impressao)
                    VALUES ('cartao_dependente', '$id_dependente', '$agora')";
    ...
    VALUES ('$id_cliente', '$id_usuario_s', 'clientes', 'Exibir cart�o para impressao - id_cliente_dependente: $id_dependente - cert_titular: $cert', '$agora')";   
...
echo "<div id=\"apDiv0\">TITULAR:</div> <div id=\"apDiv1\">".reduzirNome($nome, 40)."</div>
<div id=\"apDiv5\">".reduzirNome($nome_dependente, 30)."</div>
<div id=\"apDiv2\">$partes_chave_total</div>
<div id=\"apDiv3\">".converte_data($dt_nascimento_dependente)."</div>
<div id=\"apDiv4\">$nome_plano</div>
<div id=\"apDiv6\">DEPENDENTE</div>";
